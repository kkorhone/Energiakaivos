...
if params.t_simulation <= 0
    error('Simulation duration must be positive.');
end

if params.T_discharging <= 0
    error('Discharging temperature must be positive.');
end

if isfield(params, 'Q_discharging')
    if params.Q_discharging <= 0
        error('Discharging power must be positive.');
    end
end
...
ball_selection.set('r', r_field+2*dr_field);
...
field_volume_integration_operator = comp.cpl.create('field_volume_integration_operator', 'Integration');
field_volume_integration_operator.label('Field Volume Integration Operator');
field_volume_integration_operator.selection.geom(geom.tag, 3);
field_volume_integration_operator.selection.named(sprintf('%s_%s', geom.tag, field_volume_selection.tag));
...
for i = 1:length(bhe_array)
    if isfield(params, 'Q_discharging')
        bhe_array{i}.createBoundaryConditions(geom, phys, sprintf('if((T_outlet-delta_T)>%f[degC],T_outlet-delta_T,%f[degC])', params.T_discharging, params.T_discharging));
    else
        bhe_array{i}.createBoundaryConditions(geom, phys, sprintf('%f[degC]', params.T_discharging));
    end
end
...
vars.set('V_field', '4*field_volume_integration_operator(1)');
