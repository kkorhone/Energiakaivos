...
params.t_discharging = 100;
params.t_simulation = 1000000;

model = init_quarter_symmetry_hemispherical_recovery('ico_field_136_300m.txt', params);

mphsave(model, 'recovery.mph');

model.mesh('mesh').run();

mphsave(model, 'recovery_meshed.mph');

% model.sol('sol1').runAll();
% 
% mphsave(model, 'recovery_solved.mph');
