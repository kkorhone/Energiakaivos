% This function uses the CoaxialBoreholeHeatExchangerRecovery class.

function model = init_quarter_symmetry_hemispherical_recovery(file_name, params)

clear CoaxialBoreholeHeatExchangerRecovery % Resets the persistent id variable.
...
if params.t_discharging <= 0
    error('Discharging duration must be positive.');
end

if params.t_simulation <= params.t_discharging
    error('Simulation duration must be longer than discharging duration.');
end
...
for i = 1:length(bhe_factors)
    ...
    if (abs(bhe_collars(i, 1)) < 1e-6) && (abs(bhe_collars(i, 2)) < 1e-6)
        ...
        if is_vertical
            ...
            bhe_array{i} = CoaxialBoreholeHeatExchangerRecovery(bhe_collars(i,:), bhe_footers(i,:), params.d_borehole, coaxial_pipe, params.flow_rate, working_fluid, 'bufferradius', params.r_buffer, 'cutplanes', cut_planes);
    ...
    elseif abs(bhe_collars(i, 1)) < 1e-6
        ...
        if is_vertical
            ...
            bhe_array{i} = CoaxialBoreholeHeatExchangerRecovery(bhe_collars(i,:), bhe_footers(i,:), params.d_borehole, coaxial_pipe, params.flow_rate, working_fluid, 'bufferradius', params.r_buffer);
...
encompassing_work_plane = geom.create('work_plane', 'WorkPlane');
encompassing_work_plane.set('quickplane', 'xz');
encompassing_work_plane.set('unite', true);

circle = encompassing_work_plane.geom.create('circle', 'Circle');
circle.set('pos', [0 z_field]);
circle.set('rot', -90);
circle.set('r', r_field+dr_field);
circle.set('angle', 90);

rectangle = encompassing_work_plane.geom.create('rectangle', 'Rectangle');
rectangle.set('pos', [0 z_field]);
rectangle.set('size', [r_field+dr_field dr_field]);

union = encompassing_work_plane.geom.create('union', 'Union');
union.set('intbnd', false);
union.selection('input').set({char(circle.tag) char(rectangle.tag)});

convert = encompassing_work_plane.geom.create('convert', 'ConvertToCurve');
...
revolve.selection('input').set({char(encompassing_work_plane.tag)});
...
if z_max == 0
    ...
    surface_heat_flux_bc.label('Surface Heat Flux BC');
...
for i = 1:length(bhe_array)
    bhe_array{i}.createBoundaryConditions(geom, phys, sprintf('if(is_discharging,%f[degC],%s.Tvar)', params.T_discharging, phys.tag));
end
...
% -------------------------------------------------------------------------
% Creates encompassing surface operators.
% -------------------------------------------------------------------------
...
tlist = sprintf('0 %.0f', params.t_simulation);
...
discrete_states.label('Discrete States');
discrete_states.setIndex('dim', 'is_discharging', 0, 0);
discrete_states.setIndex('dimInit', 0, 0, 0);
discrete_states.setIndex('dim', 'is_recovering', 1, 0);
...
discharging_event.set('start', '0[a]');
discharging_event.set('period', sprintf('%f[a]', params.t_simulation));
discharging_event.setIndex('reInitName', 'is_discharging', 0, 0);
discharging_event.setIndex('reInitValue', 0, 0, 0);
discharging_event.setIndex('reInitValue', 1, 0, 0);
discharging_event.setIndex('reInitName', 'is_recovering', 1, 0);
discharging_event.setIndex('reInitValue', 0, 1, 0);

recovering_event = events.create('recovering_event', 'ExplicitEvent', -1);
recovering_event.label('Recovering Event');
recovering_event.set('start', sprintf('%f[a]', params.t_discharging));
recovering_event.set('period', sprintf('%f[a]', params.t_simulation));
recovering_event.setIndex('reInitName', 'is_discharging', 0, 0);
recovering_event.setIndex('reInitValue', 0, 0, 0);
recovering_event.setIndex('reInitName', 'is_recovering', 1, 0);
recovering_event.setIndex('reInitValue', 0, 1, 0);
recovering_event.setIndex('reInitValue', 1, 1, 0);
