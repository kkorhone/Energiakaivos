function model = init_bhe_field(num_rows, num_cols, borehole_length, borehole_spacing)
...
union = cell(1, num_rows*num_cols);
k = 1;
N = floor(num_rows/2);
M = floor(num_cols/2);
for i = 1:num_rows
    for j = 1:num_cols
        ...
        model.component('model_comp').selection(tag).set('pos', {sprintf('-%d*borehole_spacing', I) sprintf('-%d*borehole_spacing', J) '0'});
        model.component('model_comp').selection(tag).set('condition', 'inside');
        k = k + 1;
    end
end
model.component('model_comp').selection.create('uni1', 'Union');
model.component('model_comp').selection('uni1').set('entitydim', 2);
model.component('model_comp').selection('uni1').set('input', union);
