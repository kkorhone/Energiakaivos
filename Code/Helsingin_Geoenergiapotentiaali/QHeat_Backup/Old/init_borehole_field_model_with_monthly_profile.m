function model = init_single_well_model_monthly_profile(annual_heating_demand, borehole_spacing)

% Prints out summary of function arguments --------------------------------

fprintf(1, 'annual_heating_demand = %.3f MWh/a\n', annual_heating_demand);
...
model.param.set('annual_heating_demand', sprintf('%.3f[MWh]', annual_heating_demand));
model.param.set('borehole_spacing', sprintf('%.3f[m]', borehole_spacing));
model.param.set('hours_per_month', '1[a]/12');
...
monthly_profile_function = model.func.create('monthly_profile_function', 'Piecewise');
monthly_profile_function.set('funcname', 'monthly_profile');
monthly_profile_function.set('extrap', 'periodic');
monthly_profile_function.set('pieces', { '0/12'  '1/12' '0.175';  ...
                                         '1/12'  '2/12' '0.107';  ...
                                         '2/12'  '3/12' '0.112';  ...
                                         '3/12'  '4/12' '0.083';  ...
                                         '4/12'  '5/12' '0.045';  ...
                                        ...
                                        '10/12' '11/12' '0.119';  ...
                                        '11/12' '12/12' '0.123'});
monthly_profile_function.set('argunit', 'a');
monthly_profile_function.set('fununit', '1');
monthly_profile_function.label('Monthly Profile Function');
...
component_variables.set('T_inlet', 'inlet_average_operator(T)');
component_variables.set('T_outlet', 'outlet_average_operator(T)');
component_variables.set('T_bottom', 'bottom_average_operator(T)');
component_variables.set('heat_extraction_rate', 'annual_heating_demand*monthly_profile(t)/hours_per_month');
component_variables.set('delta_T', 'heat_extraction_rate/(rho_fluid*Cp_fluid*Q_fluid)');
...
inlet_average_operator = model_component.cpl.create('inlet_average_operator', 'Average');
inlet_average_operator.selection.geom('model_geometry', 2);
inlet_average_operator.selection.set([13]);
inlet_average_operator.label('Inlet Average Operator');
...
inlet_temperature_boundary.set('T0', 'T_outlet-delta_T');
...
model.study('std1').feature('time').set('tlist', 'range(0,dt,5)');

model.sol('sol1').attach('std1');
model.sol('sol1').feature('v1').set('clist', {'range(0,dt,5)' '0.005[a]'});
model.sol('sol1').feature('t1').set('tunit', 'a');
model.sol('sol1').feature('t1').set('tlist', 'range(0,dt,5)');
model.sol('sol1').feature('t1').set('maxstepconstraintbdf', 'const');
model.sol('sol1').feature('t1').set('maxstepbdf', 'dt');
