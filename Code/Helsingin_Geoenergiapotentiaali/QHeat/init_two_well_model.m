function model = init_two_well_model(borehole_spacing)
...
model.param.set('L_borehole', '2[km]'); % QHeat
model.param.set('r_borehole', '190[mm]/2'); % QHeat

model.param.set('r_outer', '114[mm]/2'); % QHeat
model.param.set('r_inner', '76[mm]/2'); % QHeat

model.param.set('T_surface', '4[degC]'); % QHeat
model.param.set('q_geothermal', '52.5[mW/m^2]'); % QHeat

model.param.set('T_inlet', '2[degC]'); % QHeat

model.param.set('k_rock', '3[W/(m*K)]'); % QHeat
model.param.set('Cp_rock', '860[J/(kg*K)]'); % QHeat
model.param.set('rho_rock', '2600[kg/m^3]'); % QHeat

model.param.set('k_pipe', '0.02[W/(m*K)]'); % Was QHeat
model.param.set('Cp_pipe', '1900[J/(kg*K)]');
model.param.set('rho_pipe', '960[kg/m^3]');

model.param.set('k_fluid', '0.58[W/(m*K)]'); % Water at 10 degC
model.param.set('rho_fluid', '1000[kg/m^3]');
model.param.set('Cp_fluid', '4190[J/(kg*K)]');

model.param.set('m_fluid', '2.5[kg/s]'); % QHeat
...
bottom_bedrock_block.set('pos', {'-(W_model+0.5*borehole_spacing)' '0' '-H_model'});
bottom_bedrock_block.set('size', {'W_model+0.5*borehole_spacing' 'W_model' 'H_model-L_borehole'});
...
component_variables.set('borehole_wall_heat_rate', '2*borehole_wall_integration_operator(ht.ndflux)');
...
borehole_wall_integration_operator.selection.set([11 25]);
...
boundary_layer_mesh_properties.selection.set([11 25]); % Boundaries
